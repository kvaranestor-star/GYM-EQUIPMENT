...
(function () {
  ...
  const SITE_DEFAULT = { hero:"", hero_tag:"FLAGSHIP", hero_brand:"SHUA × YR", look1:"", look2:"", look3:"" };
  ...
  async function siteSave(data) {
    const clean = {
      hero: cut(data.hero, 600), hero_tag: cut(data.hero_tag, 40), hero_brand: cut(data.hero_brand, 40),
      look1: cut(data.look1, 600), look2: cut(data.look2, 600), look3: cut(data.look3, 600)
    };
    if (online()) {
  ...
  /* ---------------- QUOTES (gym layout requests) ---------------- */
  const Q = "creagym_quotes";
  function normQuote(q) {
    return {
      name: cut(q.name, 80), phone: cut(q.phone, 30), comment: cut(q.comment, 600),
      total: Math.max(0, Math.round(+q.total || 0)),
  ...
  async function quoteList() {
    if (online()) { const r = await fetch(URL() + "/rest/v1/quotes?select=*&order=created_at.desc", { headers: headers() }); if (!r.ok) throw new Error("Supabase " + r.status); return await r.json(); }
    return lsGet(Q, []);
  }
  async function quoteUpdate(id, patch) {
    if (online()) { const r = await fetch(URL() + "/rest/v1/quotes?id=eq." + encodeURIComponent(id), { method: "PATCH", headers: Object.assign(headers(), { Prefer: "return=minimal" }), body: JSON.stringify(patch) }); return r.ok; }
    const a = lsGet(Q, []); const i = a.findIndex(x => String(x.id) === String(id)); if (i > -1) { Object.assign(a[i], patch); lsSet(Q, a); } return true;
  }
  async function quoteDelete(id) {
    if (online()) { const r = await fetch(URL() + "/rest/v1/quotes?id=eq." + encodeURIComponent(id), { method: "DELETE", headers: headers() }); return r.ok; }
    lsSet(Q, lsGet(Q, []).filter(x => String(x.id) !== String(id))); return true;
  }
  ...
  window.CREAGYM_API = {
    ...
    quoteCreate, quoteList, quoteUpdate, quoteDelete
  };
})();
